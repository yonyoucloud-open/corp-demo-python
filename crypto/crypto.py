...
class Crypto:
    ...
    def __init__(self, app_key: 'str', app_secret: 'str'):
        ...
        :param app_key:     自建应用的 app key
        :param app_secret:  自建应用的 app secret
        """
        self.app_key = app_key
        self.app_secret = app_secret
        self.aes_key = _aes_key_from_secret(app_secret)
    ...
    def sign(self, encrypted: 'str', timestamp: 'int', nonce: 'str'):
        ...
        :return:
        """
        array = [self.app_secret, str(timestamp), nonce, encrypted]
        array.sort()
        sort_str = ''
        for s in array:
            sort_str += s
    ...
    def _decode_aes(self, aes_data: 'bytes'):
        ...
        if from_key.decode() != self.app_key:
            raise RuntimeError('app key 不匹配, 需要：' + self.app_key + '，实际：' + from_key.decode())
        return message


def _aes_key_from_secret(app_secret: 'str'):
    replaced_secret = app_secret.replace('-', '')
    while len(replaced_secret) < 43:
        replaced_secret += '0'
    replaced_secret += '='
    return base64.b64decode(replaced_secret)
